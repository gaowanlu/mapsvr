...
function Map3DOctree.ContainsNode(node, obj)
    return (obj.pos.x >= node.x and obj.pos.x < node.x + node.w and obj.pos.y >= node.y and obj.pos.y < node.y + node.h
        and obj.pos.z >= node.z and obj.pos.z < node.z + node.d)
end
...
function Map3DOctree.Intersect(a, b)
    return not (b.x > a.x + a.w or a.x > b.x + b.w or b.y > a.y + a.h or a.y > b.y + b.h or b.z > a.z + a.d
        or a.z > b.z + b.d)
end
...
function Map3DOctree.OcQuery(node, range, out, seen)
    ...
    for _, obj in ipairs(node.list) do
        repeat
            if not (obj.pos.x >= range.x and obj.pos.x < range.x + range.w
                and obj.pos.y >= range.y and obj.pos.y < range.y + range.h
                and obj.pos.z >= range.z and obj.pos.z < range.z + range.d) then
                break
            end
