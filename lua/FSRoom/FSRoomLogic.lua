...
function FSRoom.new(roomId, maxPlayers)
    ...
    -- 模拟FSRoom的DB字段
    self.FSRoomDbData = { id = roomId, name = "FSRoom_" .. tostring(roomId) }
...
function FSRoom:FinishGame(winnerUserId, reason)
    ...
    Log:Error(
        "Game finished in room %s winnerUserId %s Reason %s", tostring(self.FSRoomDbData.id), winnerUserId or "none",
        reason or "completed"
    )
...
function FSRoom:HandleReconnect(playerId, userId, lastFrameId)
    ...
    Log:Error(
        "Player userId %s reconnected to room %s, missed frames %d", userId, tostring(self.FSRoomDbData.id),
        #missedFrames
    )
