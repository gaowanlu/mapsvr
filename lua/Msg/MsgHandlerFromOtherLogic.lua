...
MsgHandlerFromOther[ProtoLua_ProtoCmd.PROTO_CMD_CS_REQ_EXAMPLE] = function (cmd, message, app_id)
    ...
    local protoCSResExample = { testContext = message.testContext }

    MsgHandler:Send2IPC(app_id, ProtoLua_ProtoCmd.PROTO_CMD_CS_RES_EXAMPLE, protoCSResExample)
end
...
MsgHandlerFromOther[ProtoLua_ProtoCmd.PROTO_CMD_DBSVRGO_INSERT_DBUSERRECORD_RES] = function (cmd, message, app_id)
    ...
    MsgHandler:Send2Client(
        message.clientGID, message.workerIdx, ProtoLua_ProtoCmd.PROTO_CMD_CS_RES_CREATE_USER, protoCSResCreateUser
    )
end
...
MsgHandlerFromOther[ProtoLua_ProtoCmd.PROTO_CMD_DBSVRGO_SELECT_DBUSERRECORD_LOGIN_RES] = function (cmd, message, app_id)
    ...
    -- 判断密码是否正确
    ---@type ProtoLua_ProtoCSResLogin
    local protoCSResLogin = { ret = ProtoLua_ProtoErrCode.OK, sessionId = playerId }
