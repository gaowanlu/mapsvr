...
---@class Map3DBedConfig
---@field x     number  中心X(相对地图中心)
---@field z     number  中心Z(相对地图中心)
---@field w     number  宽(X方向)
---@field d     number  深(Z方向)
---@field rotY  number  Y轴朝向角(弧度, 决定床头方向, 客户端据此摆放枕头)
---@field color integer 颜色(0xRRGGBB)

---@class Map3DMapConfigType
---@field name  string
---@field boxes table<integer, Map3DBoxConfig>
---@field beds  table<integer, Map3DBedConfig> | nil 地图内的床(可上床睡觉, 一床一人)
...
local function buildArena4()
    ...
    local function addBox(x, yRel, z, w, h, d, color, collide)
        boxes[#boxes + 1] = { x = x, y = yRel, z = z, w = w, h = h, d = d, color = color, collide = collide ~= false }
    end
    ...
    local function building(x, z, w, d, h, color, roof, doors)
        ...
        if doors.n then
            wallHGap(L, R, T, x, gap, h, t, color)
        else
            wallH(L, R, T, h, t, color)
        end
        if doors.s then
            wallHGap(L, R, B, x, gap, h, t, color)
        else
            wallH(L, R, B, h, t, color)
        end
        if doors.w then
            wallVGap(T, B, L, z, gap, h, t, color)
        else
            wallV(T, B, L, h, t, color)
        end
        if doors.e then
            wallVGap(T, B, R, z, gap, h, t, color)
        else
            wallV(T, B, R, h, t, color)
        end
    ...
    -- ---- 7. 床(可上床睡觉, 一床一人; 不阻挡移动, 服务器权威占用) ----
    -- 放在四个象限建筑内, 远离中央出生点
    ---@type table<integer, Map3DBedConfig>
    local beds = {}
    local M_BED = 0x7a5c3e                                                                     -- 床 暖棕
    beds[#beds + 1] = { x = -32, z = -38, w = 2.2, d = 3.6, rotY = 0, color = M_BED }          -- A 西北建筑(靠北墙, 床头朝西)
    beds[#beds + 1] = { x = 32, z = -38, w = 2.2, d = 3.6, rotY = math.pi, color = M_BED }     -- B 东北建筑(床头朝东)
    beds[#beds + 1] = { x = -32, z = 38, w = 2.2, d = 3.6, rotY = math.pi / 2, color = M_BED } -- C 西南建筑(床头朝南)
    beds[#beds + 1] = { x = 32, z = 38, w = 2.2, d = 3.6, rotY = -math.pi / 2, color = M_BED } -- D 东南建筑(床头朝北)

    return { name = "ARENA-4", boxes = boxes, beds = beds }
end
